...
export const TYPES={salary:'Monthly Salary',income_other:'Other Income',expense:'Expense',transfer:'Transfer',withdraw:'Withdraw Cash',etransit_add:'E-Transit Top-up',savings_add:'Savings Added',savings_use:'Savings Used',borrow:'Borrowed Money',repay:'Repayment'};
...
export function balances(transactions,upto){
 ...
 for(const t of transactions.filter(x=>!upto||x.date<=upto)){
  ...
  if(['salary','income_other'].includes(t.type)){t.method==='cash'?cash+=a:online+=a}
...
export function stats(transactions,m){
 ...
 const income=sum(tx,t=>['salary','income_other'].includes(t.type)?t.amount:0),expenses=sum(exp,t=>t.amount);
 ...
 tx.filter(t=>['salary','income_other'].includes(t.type)).forEach(t=>incomeByCategory[t.category||t.source||'Other Income']=(incomeByCategory[t.category||t.source||'Other Income']||0)+Number(t.amount||0));
