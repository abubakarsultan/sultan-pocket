...
export function lendingSnapshot(transactions, upto) {
  ...
  Object.values(people).forEach(v => { v.remaining = Math.max(0, v.lent - v.recovered); });
  return people;
}
...
export function balances(transactions, upto) {
  ...
  let borrowed=0, repaid=0, lent=0, recovered=0;
  ...
  const lending = lendingSnapshot(list, upto);
  const owed = Object.values(debt).reduce((a,v) => a + v.remaining, 0);
  const receivable = Object.values(lending).reduce((a,v) => a + v.remaining, 0);
  return {
    cash, online, etransit, savings,
    borrowed, repaid, owed, lent, recovered, receivable,
...
export function stats(transactions, m) {
  ...
  const lent = sum(tx, t => t.type === 'lend' ? t.amount : 0);
  const recovered = sum(tx, t => t.type === 'lend_repay' ? t.amount : 0);
  ...
  return {
    ...
    lendingByPerson: lendingSnapshot(transactions, monthCutoff(m)),
    lent, recovered,
