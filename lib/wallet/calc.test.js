...
import {balances,stats,debtSnapshot,lastDayOfMonth,firstDayOfMonth,money,monthSummary} from './calc.js';
...
test('money formats supported currencies and keeps PKR default',()=>{
  assert.equal(money(1000,'USD'),'$ 1,000');
  assert.equal(money(1000),'Rs. 1,000');
});


test('monthSummary returns opening through closing figures',()=>{
  const tx=[
    {id:'1',type:'salary',amount:50000,date:'2026-08-01',method:'online'},
    {id:'2',type:'expense',amount:5000,date:'2026-08-02',method:'online',category:'Food'},
    {id:'3',type:'savings_add',amount:10000,date:'2026-08-03',method:'online'},
  ];
  const summary=monthSummary(tx,'2026-08');
  assert.equal(summary.income,50000);
  assert.equal(summary.expenses,5000);
  assert.equal(summary.savings,10000);
  assert.equal(summary.transport,0);
  assert.equal(summary.opening,0);
  assert.equal(summary.closing,45000);
});

test('lend and receive repayment track receivables without counting them as income or expense',()=>{
  const tx=[
    {id:'1',type:'salary',amount:10000,date:'2026-08-01',method:'cash'},
    {id:'2',type:'lend',amount:3000,date:'2026-08-02',method:'cash',person:'Ali'},
    {id:'3',type:'lend_repay',amount:1000,date:'2026-08-05',method:'cash',person:'Ali'},
  ];
  const b=balances(tx);
  assert.equal(b.cash,8000);
  assert.equal(b.receivable,2000);
  assert.equal(b.lent,3000);
  assert.equal(b.recovered,1000);
  const s=stats(tx,'2026-08');
  assert.equal(s.income,10000);
  assert.equal(s.expenses,0);
  assert.equal(s.lendingByPerson.Ali.remaining,2000);
});
