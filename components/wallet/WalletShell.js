...
export default function WalletShell({ children, title = 'Manage Your Money' }) {
  const p = usePathname();
  const r = useRouter();
  ...
  const protectedAction = (fn) => {
    if (user || (guest && !guestLimitReached)) fn();
    else { setGateReason('limit'); setGate(true); }
  };
    ...
    <>
      ...
      <div className="wallet-shell">
        <aside className="wallet-sidebar">
          <Link href={basePath} className="wallet-brand"><span>💳</span><b>Manage Your Money</b></Link>
          <nav>
            {NAV.map(([href, ic, label]) => (
              <Link className={p === href ? 'active' : ''} key={href} href={href}><i>{ic}</i>{label}</Link>
            ))}
          </nav>
          <div className="wallet-side-foot">
            <Link href={user ? '/dashboard' : '/'}>{user ? '← Main dashboard' : '← Sultan Pocket home'}</Link>
            {user ? (
              <Link href={`/u/${encodeURIComponent(user.user_metadata?.username || user.email?.split('@')[0] || 'user')}`}>View profile</Link>
            ) : (
              <button onClick={() => {setGateReason('guest');setGate(true)}}>Guest mode · Create account</button>
            )}
          </div>
        </aside>
        ...
        <section className="wallet-main">
          ...
          <button type="button" className="wallet-mobile-fab" aria-label="Add expense" onClick={() => protectedAction(() => window.dispatchEvent(new CustomEvent('wallet:add', { detail: 'expense' })))}>+</button>

          <nav className="wallet-mobile-nav" aria-label="Wallet navigation">
            {NAV.map(([href, ic, label]) => (
              <Link className={p === href ? 'active' : ''} key={href} href={href}>
                <i aria-hidden="true">{ic}</i>
                <span>{label === 'Debt & Borrowing' ? 'Debt' : label === 'Charts & Stats' ? 'Charts' : label}</span>
              </Link>
            ))}
          </nav>
        </section>

        {gate && <GateModal close={() => setGate(false)} router={r} basePath={basePath} reason={gateReason} />}
      </div>
    </>
  );
}
  ...
  return (
    <div className="gate-backdrop">
      <div className="gate-modal">
        ...
        <span className="wallet-modal-kicker">{reason === 'limit' ? 'GUEST LIMIT' : 'GUEST MODE'}</span>
        <h2>{reason === 'limit' ? 'Ready to save more?' : 'Use Sultan Pocket for free'}</h2>
        <p>{reason === 'limit' ? 'You can try Sultan Pocket as a guest, but guest storage is limited. Create a free account to save unlimited transactions and keep your data across devices.' : 'Try the Expense Tracker without signing up. Your guest transactions are saved only on this device. Create an account when you are ready for permanent cloud storage.'}</p>
        <div className="gate-buttons">
          <button className="wallet-btn primary" onClick={() => router.push(`/signin?next=${encodeURIComponent(basePath)}`)}>Sign in</button>
          <button className="wallet-btn" onClick={() => router.push(`/signup?next=${encodeURIComponent(basePath)}`)}>Create account</button>
        </div>
      </div>
    </div>
  );
