...
const INCOME=['Monthly Salary','Family','Other Income'];
...
export default function WalletModal(){
 ...
 useEffect(()=>{
  function handleAdd(e){if(!user)return;const detail=e.detail||'expense';const nextKind=typeof detail==='string'?detail:detail.type||'expense';setEditingId(null);setKind(nextKind);setNewCategory('');setForm({...BASE,...(typeof detail==='object'?detail:{}),type:nextKind,date:today(),category:nextKind==='income'?'Monthly Salary':nextKind==='expense'?(detail?.category||'Food'):'Food',from:nextKind==='withdraw'?'online':(detail?.from||'cash'),method:nextKind==='expense'&&detail?.category==='Transport'?(detail?.method||'etransit'):(detail?.method||'cash')});setOpen(true)}
  function handleEdit(e){if(!user||!e.detail)return;const t=e.detail,k=normalizeType(t.type);setEditingId(t.id);setKind(k);setNewCategory('');setForm({...BASE,...t,type:k,amount:String(t.amount??''),date:t.date||today(),category:t.category||(k==='income'?'Monthly Salary':'Food'),method:t.method||'cash',from:t.from||'cash',destination:t.destination||'cash',repayRequired:t.repayRequired!==false});setOpen(true)}
