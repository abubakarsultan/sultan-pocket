...
export default function WalletModal(){
  ...
  useEffect(()=>{
    ...
    function handleEdit(e){
      if(!user&&!guest)return;const t=e.detail;const k=normalizeType(t.type);
      setEditingId(t.id);setKind(k);setNewCategory('');setAttachmentFile(null);setAttachmentRemove(false);setAttachmentError('');
      if(attachmentInputRef.current)attachmentInputRef.current.value='';
      setForm({...BASE,...t,type:k,amount:String(t.amount??''),date:t.date||todayISO(),category:t.category||(k==='income'?'Monthly Salary':'Food'),method:t.method||'cash',from:t.from||'cash',destination:t.destination||'cash',repayRequired:t.repayRequired!==false,attachment_path:t.attachment_path||'',goal_id:t.goal_id||t.details?.goal_id||''});
      setOpen(true);
    }
  ...
  function handleAttachmentChange(e){
    ...
    if(file.size>5*1024*1024){setAttachmentError('Receipt image must be 5 MB or smaller.');e.target.value='';return;}
    setAttachmentFile(file);setAttachmentRemove(false);
  }
  function removeAttachment(){setAttachmentFile(null);setAttachmentRemove(true);setAttachmentError('');if(attachmentInputRef.current)attachmentInputRef.current.value='';}
  async function uploadAttachment(transactionId,file){
    ...
    const path=`${user.id}/${transactionId}-${Date.now()}.${ext}`;const {error}=await supabase.storage.from('wallet-attachments').upload(path,file,{contentType:file.type,upsert:false});
    if(error){notify(error.message||'Could not upload the attachment.');return null;}return path;
  }
  ...
  return <div className="gate-backdrop"><form className="wallet-modal" onSubmit={submit}>
    ...
    <div className="wallet-form-grid">
      ...
      {(isExpense||isIncome)&&<>
        ...
        <label className="full category-helper">New {isIncome?'income':'expense'} category<div className="category-create"><input id="wallet-new-category" value={newCategory} onChange={e=>setNewCategory(e.target.value)} placeholder={isIncome?'e.g. Freelance Income':'e.g. Medical'}/><button type="button" className="wallet-btn secondary" onClick={saveCategory}>Save category</button></div><small>This category will only appear in {isIncome?'Income':'Expense'} transactions.</small></label>
      </>}
      ...
      {isSavings&&<><label>Savings goal<select value={form.goal_id||''} onChange={e=>setField('goal_id',e.target.value)}><option value="">General savings</option>{goals.map(goal=><option key={goal.id} value={goal.id}>{goal.name}</option>)}</select></label><label>{kind==='savings_add'?'Save from':'Return to'}<select value={kind==='savings_add'?form.method:form.destination} onChange={e=>setField(kind==='savings_add'?'method':'destination',e.target.value)}><option value="cash">Cash</option><option value="online">Online</option></select></label></>}
    ...
    <div className="wallet-modal-foot"><button type="button" className="wallet-btn secondary" onClick={close} disabled={busy}>Cancel</button><button className="wallet-btn primary" disabled={busy}>{busy?(editingId?'Updating…':'Saving…'):(editingId?'Update transaction':'Save transaction')}</button></div>
  </form></div>;
}
