  ...
  return <footer className="site-footer">
    <div className="container footer-grid">
      <div className="footer-brand-col">
        <Link href="/" className="footer-brand"><Image src="/logo.png" alt="Sultan Pocket" width={36} height={36} /><strong>Sultan Pocket</strong></Link>
        <p>Personal finance, made simple. Track your money, understand your spending, and keep your wallet organized.</p>
        <div className="footer-socials" aria-label="Social media">
          {socials.length ? socials.map(([label,url])=><a key={label} href={url} target="_blank" rel="noreferrer" aria-label={label}>{label}</a>) : <span className="footer-social-placeholder">Social links will appear here when configured in Admin → SEO.</span>}
        </div>
        <FooterNewsletter />
      </div>
