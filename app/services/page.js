...
export default function ServicesPage() {
  return <main>
    <ScrollReveal as="section" className="page-hero"><div className="container narrow"><span className="section-label">SULTAN POCKET FEATURES</span><h1>One wallet for the way you actually use money.</h1><p>From your monthly salary to your daily coffee, Sultan Pocket helps you record the movement of your money clearly.</p><div className="hero-actions"><Link href="/signup" className="btn btn-primary">Get started free →</Link><Link href="/expense-tracker" className="btn">Open wallet</Link></div></div></ScrollReveal>
    <ScrollReveal as="section" className="section-block"><div className="container"><div className="service-grid">{services.map(([icon,title,desc,href]) => <Link href={href} className="service-card" key={title}><span className="feature-icon">{icon}</span><h2>{title}</h2><p>{desc}</p><span className="feature-link">Learn more →</span></Link>)}</div></div></ScrollReveal>
    <ScrollReveal as="section" className="final-cta"><div className="container"><h2>Your money. One clear place.</h2><p>Start with the wallet today and build better financial habits over time.</p><Link href="/signup" className="btn btn-primary">Create your account →</Link></div></ScrollReveal>
  </main>;
}
