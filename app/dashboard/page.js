...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (user) supabase.rpc('am_i_admin').then(({ data }) => setIsAdmin(!!data));
  }, [loading, user, router]);
    ...
    <main className="container" style={{ padding: '48px 24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12, marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: 'var(--text-faint)' }}>Choose a tool to get started.</p>
        </div>
        {isAdmin && (
          <a href="/dashboard/admin/blog" className="btn btn-primary">Blog admin</a>
        )}
      </div>
