...
export default function ProfilePage(){
  ...
  async function exportMyData(){
    setBusy(true);setError('');setNotice('');
    try{
      const [{data:transactions,error:txError},{data:categories,error:catError},{data:recurring,error:recError}]=await Promise.all([
        supabase.from('wallet_transactions').select('*').eq('user_id',user.id).order('date',{ascending:true}).order('created_at',{ascending:true}),
        supabase.from('wallet_categories').select('*').eq('user_id',user.id).order('created_at',{ascending:true}),
        supabase.from('wallet_recurring').select('*').eq('user_id',user.id).order('created_at',{ascending:true})
      ]);
  ...
  return <main className="profile-page"><section className="profile-card">
    ...
    {editing?<div className="profile-form">
      ...
      <section id="account-actions" className="profile-danger"><h2>Account actions</h2><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><button className="btn profile-signout-btn" onClick={signOut} disabled={busy}>Sign out</button><button className="btn" onClick={exportMyData} disabled={busy}>Export My Data (CSV)</button><button className="btn profile-delete-btn" onClick={deleteAccount} disabled={busy}>Delete account</button></div><p className="profile-help">Deleting your account permanently removes your account and Wallet data.</p></section>
    </div>}
  </section></main>;
}
